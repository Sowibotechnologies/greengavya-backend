...
router.get("/getcountall", function(req, res){
        db.query("SELECT (SELECT COUNT(*) FROM products) as product_count,(SELECT COUNT(*) FROM product_category WHERE category_id !='NONE') as category_count,(SELECT COUNT(*) FROM orders INNER JOIN user ON orders.userid = user.user_id WHERE status='Pending') as order_count,(SELECT COUNT(*) FROM user) as user_count", function(err, result){
                if(err){
                        console.log(err);
                }
