...
class ShowBlacklist extends React.Component{
    state={
        isFetching: true,
        users:{},
        currentPage: 1,
    ...
    fetchUsers(){
        ...
        fetch("/api/user/getallblacklist")
        .then(res => res.json())
        .then(result => {
            this.setState({
                isFetching:false,
            ...
            <React.Fragment>
                <div class="container card table-responsive" style={{marginTop:'20px',height:'865px'}}>
                    <div className="text-center">
                        <img alt="Blacklist icon" className="card-head-img" src="/images/blacklist.png"/>
                        <span className="card-head-title">BlackList</span>
                    </div>
                    {this.state.isFetching ? <Loader/>: (this.state.users.length === 0 ? <Empty/>:
                        (<table className="table" style={{marginTop:'30px'}}>
                                <tbody>
                                    ...
                                    <th className="cissa-table-header">No of orders&nbsp;&nbsp;<span style={{cursor:'pointer'}} className={this.state.orderToggle === ''? "glyphicon glyphicon-menu-down toggler" : (this.state.orderToggle === 'down' ? "glyphicon glyphicon-menu-down" : "glyphicon glyphicon-menu-up")} onClick={this.sortArrayByOrder.bind(this)}></span></th>
                    ...
                    <div class="modal-dialog modal-sm">
                        ...
                        <div className="text-left">
                            <div class="text-center modal-body">
                                <img alt="Unblock icon" src="/images/unblock.png" width='75'></img>
                                <h1 style={{color:'#a9a2a2',fontWeight:'100'}}>Are you sure?</h1>
                                <form id="deletecategory" style={{marginTop:'20px'}}>
                                    <i style={{color:'#a9a2a2',fontWeight:'100',fontSize:'16px'}}>This user will be whitelisted !</i>
                                    <div class="row text-center" style={{marginTop:'20px'}}>
                                        ...
                                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                            <button type="button" className="modal-btn success" data-dismiss="modal" onClick={this.unblockUser.bind(this)}>UnBlock</button>
                                        </div>
                                    </div>
                                </form>  
                            </div>
...
const UserRow = (props) =>{
    return(
        <tr className="showProduct-tableBody left-cell">
            ...
            <td style={{textAlign:'center'}}>{props.data.order_count}</td>
            <td style={{textAlign:'center'}}>
                <label class="switch cissa-tooltip" >
                    <img alt="Unblock icon" src="/images/unblock.png" className="showProduct-imgBtn" data-toggle="modal" data-target="#UserModalBlock"  onClick={()=>{
                                props.context.setState({
                                    blockID: props.data.user_id
                                })
                            }}></img>
                    <span class="tooltiptext">Unblock account</span>
                </label>
            </td>
