...
class PurchaseByOrder extends React.Component{
    state={
        order:[],
        orderId:"",
        isFetching: true
    ...
    renderOrderDetails(){
        ...
        return(
            ...
            <div className="text-left container-fluid">
                <h4>OrderNo: #{this.state.order[0].orderid}
                    <span style={{float:'right'}}><img alt="Print icon" src="/images/print.png" style={{marginLeft:'20px'}} className="showProduct-imgBtn" onClick={(e)=>{this.props.context.printOrder(e,this.state.order)}}></img></span>
                </h4>
                <h5 style={{textTransform:'capitalize'}}>Ordered by: {this.state.order[0].username}</h5>
                <h5>Ordered on: {formatted_date.toDateString()}</h5>
                <h5>Payment Method: {this.state.order[0].payment_method}</h5>
                <h5 style={{textTransform:'capitalize'}}>Delivery Speed:&nbsp;
                {this.state.order[0].delivery_speed ==="fast" && <img src="/images/fast.png" alt="ico" width="20"></img>}
                {this.state.order[0].delivery_speed}
                </h5>
            </div>
...
export default PurchaseByOrder;